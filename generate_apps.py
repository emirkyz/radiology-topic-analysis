...
<body>
    ...
    function updateChart() {{
        ...
        const option = {{
            ...
            tooltip: {{ trigger: 'item', backgroundColor: 'rgba(255,255,255,0.95)', borderColor: '#e2e8f0', borderWidth: 1, textStyle: {{ color: '#1e293b' }}, formatter: function(param) {{ return `${{param.name}}<br/><span style="color:${{param.color}}">●</span> ${{param.seriesName}}: ${{parseFloat(param.value).toFixed(4)}}`; }} }},
            ...
            yAxis: {{ type: 'value', name: 'Normalized Topic Weight', nameTextStyle: {{ color: '#64748b' }}, axisLabel: {{ color: '#64748b' }}, axisLine: {{ lineStyle: {{ color: '#e2e8f0' }} }}, splitLine: {{ lineStyle: {{ color: '#f1f5f9' }} }}, min: 0 }},
            series: series
        }};
        chart.setOption(option, true);
    }}
